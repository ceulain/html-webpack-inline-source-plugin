...
HtmlWebpackInlineSourcePlugin.prototype.resolveSourceMaps = function (
  compilation,
  assetName,
  asset
) {
...
HtmlWebpackInlineSourcePlugin.prototype.processTag = function (
  compilation,
  regex,
  tag,
  filename
  ...
  if (assetUrl) {
    ...
    if (compilation.assets[assetName] !== undefined) {
      ...
      tag.innerHTML =
        tag.tagName === "script"
          ? updatedSource.replace(/(<)(\/amp-script>)/g, "\\x3C$2")
          : updatedSource;
    } else {
      return preTag;
    }
